...
_help() {
  cat << EOF
    ...
    A helper script to connect to a mysql db
...
then
  ...
  PS3="Please select the rds instance to connect to:"$'\n'
...
if [ -z "$db_name" ]
then
  # prompt user to type the name of the database to be connected
  read -p "Please type the name of the database to connect to: " db_name
else
...
# create connect-mysql-db security group if it doesn't already exist
security_group_name="connect-mysql-db"
...
then
  ...
  security_group=$(aws $profile $region ec2 create-security-group --group-name $security_group_name --description "connect-mysql-db" --vpc-id $rds_instance_vpc)
...
cat << EOF > connect-mysql-db-task-role-trust-policy.json
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
task_role=$(aws $profile $region iam get-role --role-name connect-mysql-db-task-role --query 'Role.Arn' --output text)
if [ -z "$task_role" ]
then
  echo "task role does not exist, creating it"
  # create task role
  task_role=$(aws $profile $region iam create-role --role-name connect-mysql-db-task-role --assume-role-policy-document file://connect-mysql-db-task-role-trust-policy.json | jq -r '.Role.Arn')
else
  echo "task role already exists, using it"
fi

# assign AmazonECSTaskExecutionRolePolicy to the task role
aws $profile $region iam attach-role-policy --role-name connect-mysql-db-task-role --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
# assign secretsmanager managed policy to the task role
aws $profile $region iam attach-role-policy --role-name connect-mysql-db-task-role --policy-arn arn:aws:iam::aws:policy/SecretsManagerReadWrite
# assign ssm managed policy to the task role
aws $profile $region iam attach-role-policy --role-name connect-mysql-db-task-role --policy-arn arn:aws:iam::aws:policy/AmazonSSMFullAccess

cat << EOF > connect-mysql-db-task-def.json
{
  "family": "connect-mysql-db-task-def",
  "networkMode": "awsvpc",
  "containerDefinitions": [
    {
      "name": "connect-mysql-db-container",
      "image": "mysql:8",
      "essential": true,
      "command":[
        "sh",
...
task_def=$(aws $profile $region ecs register-task-definition --cli-input-json file://connect-mysql-db-task-def.json | jq -r '.taskDefinition.taskDefinitionArn')
...
# wait for the task atatus to be equal to RUNNING
task_status=$(aws $profile $region ecs describe-tasks --cluster $cluster --tasks $task_id | jq -r '.tasks[].lastStatus')
while [ "$task_status" != "RUNNING" ]
do
  echo "task status is $task_status, waiting for it to be RUNNING"
  sleep 5
  task_status=$(aws $profile $region ecs describe-tasks --cluster $cluster --tasks $task_id | jq -r '.tasks[].lastStatus')
done
...
aws $profile $region ecs execute-command  \
    --cluster $cluster  \
    --task $task_id \
    --container "connect-mysql-db-container"\
    --command 'mysql -h $TARGET_DB_HOST -u $TARGET_DB_USER --password=$TARGET_DB_PASSWORD $TARGET_DB_NAME' \
    --interactive
