...
#end_time=$(date -d 'today 23:00:00' +%s%3N)
start_time=$(date -d '19 June 2024 10:00' +%s%3N)
end_time=$(date -d '24 June 2024 12:00' +%s%3N)
#start_time=$(date -d '-1 days' +%s%3N)
#start time is midnight yesterday
#start_time=$(date -d 'yesterday 00:00:00' +%s%3N)
#start_time=$(date -d '10/10/2023 01:00:00' +%s%3N)
#start_time=$(date -d '-4 days' +%s%3N)
...
# prompt user to type log stream prefix
read -p "Enter the log stream prefix: " log_stream_prefix




#execute filter-log-events using date and log stream prefix
result=$(aws ${profile} ${region} logs filter-log-events \
