...
echo "this is going to turn off all ecs services"
...
read -p "Are you sure you want to turn off all ecs services? (y/n) " -n 1 -r
...
do
  ...
  aws ${profile} ${region} ecs update-service --cluster $SELECTED_CLUSTER --service $SELECTED_SERVICE --desired-count 0  --query "service.[serviceName,desiredCount]"
done
